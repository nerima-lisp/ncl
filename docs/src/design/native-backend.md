...
The x86-64 target uses r15 for context, rdi for entry `argc`, rsi/rdx/rcx/r8 for the first four logical arguments, r9 for `rest`, rax for the return value, rdx for the multiple-value count, and r10/r11 as scratch. `rbp` is the frame pointer and points at header word 0, so a caller reserves 16 bytes below `rsp` before a call and the callee writes header words 2 and 3 above its frame pointer. Its fixed-template backend emits the same four-word frame header, 16-byte alignment, and safepoint map wire format as AArch64, with value slots at `-8 * (index + 1)` from the frame pointer.
...
Phase 1a's x86-64 lowering mirrors those two paths. The fast path loads `tlab_bump` and `tlab_limit`, computes `bump + words * 8` with `lea`, branches on an unsigned-above comparison, and stores the advanced bump. The slow path places the pinned context in `rdi` and the requested word count in `rsi` before calling through the `RuntimeFunction::AllocateSlow` address. Safepoint lowering loads the `safepoint_request` word and skips the slow path when it is zero; otherwise it passes the pinned context in `rdi`, the active generated frame pointer (`rbp`) in `rsi`, and the continuation PC in `rdx` (loaded with a RIP-relative `lea` whose displacement spans the indirect call that follows, so it equals the safepoint map `pc_offset`), then calls `RuntimeFunction::SafepointSlow`. Comparisons materialise their boolean with `setcc` followed by `movzx`, because `setcc` writes only the low byte. (`packages/codegen/src/target_x86_64_lowering.rs`, `lower_alloc`, `lower_safepoint`.)
