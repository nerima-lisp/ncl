...
Macro expansion is available in both routes. The compiler is an execution
backend for the current language surface; it is not an implementation of
SBCL's optimizing compiler.
