...
| Packages | Package definition and lifecycle with <code>make-package</code>, <code>delete-package</code>, and <code>rename-package</code>, use lists, nicknames, bounded shadowing/user metadata, exports, qualification, interning, lookup, and package predicates. | Full package objects, complete symbol identity/shadowing semantics, package symbol iteration, and all standard conflict/mutation behavior remain incomplete. |
| Numbers | Integers, floats, rationals, complex values, arithmetic, comparisons, rounding, integer operations, rational accessors, transcendental functions, and the tested <code>coerce</code> surface. | The complete numeric tower and all coercion/printing rules are not implemented. |
...
| Arrays and tables | Simple, multidimensional, adjustable, displaced, and fill-pointer arrays plus hash tables with <code>eq</code>, <code>eql</code>, <code>equal</code>, and <code>equalp</code> tests. | Specialized arrays, full sequence semantics, hash-table iteration protocols, and standard edge cases remain work. |
