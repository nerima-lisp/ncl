...
#[test]
fn golden_sse_and_decode_errors() {
    for (op, opcode) in [
        (SseOp::Subsd, 0x5c),
        (SseOp::Mulsd, 0x59),
        (SseOp::Divsd, 0x5e),
    ...
    assert_eq!(decode(&[0x0f]), Err(DecodeError::Unsupported));
    assert_eq!(decode(&[0x91]), Err(DecodeError::Unsupported));
    assert_eq!(display(&Inst::Ret), "Ret");
}
...
fn reachable_error_displays_and_sse_memory_are_specific() {
    ...
    for error in [
        EncodeError::Rel32OutOfRange,
        EncodeError::InvalidNopLength,
        EncodeError::BufferTooLarge,
        EncodeError::InvalidRegister(7),
    ] {
