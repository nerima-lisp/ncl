...
fn executes_cons_allocation_car_and_cdr_on_tlab_fast_path() {
    let abi = BuiltinAbi;
    let compiled = compile_function_x86_64(&build_cons_function(), &abi).expect("lowering");
    ...
    // The allocation slow path registers its map at the address after its call.
...
fn executes_safepoint_poll_without_and_with_request() {
    ...
    let end = usize::try_from(map.pc_offset).expect("map offset");
    // The continuation PC is loaded by a RIP-relative lea spanning the call that
    // follows it, so the map sits on the address after the call.
    assert_eq!(
