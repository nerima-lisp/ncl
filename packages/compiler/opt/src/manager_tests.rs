...
use crate::{FunctionPass, InlineDirectCalls, Module, ModulePass, PassError, PassManager, PassManagerOptions, PassResult};
use ncl_ir::{Constant, ConstantIndex, Function, Op, OpKind, Terminator, Ty, ValueId};

#[derive(Debug)]
struct TogglePass { changed: bool }
...
impl ModulePass for ModuleToggle {
    fn name(&self) -> &'static str { "module-toggle" }
    fn run(&mut self, _module: &mut Module) -> PassResult { Ok(false) }
}
...
impl FunctionPass for InvalidatingPass {
    fn name(&self) -> &'static str { "invalidating" }
...
fn manager_reports_module_passes_limits_and_errors() {
    let options = PassManagerOptions { max_iterations: 1, verify_after_each_pass: true };
    ...
    let report = manager.run(&mut Module { functions: vec![leaf()] }).fixture();
    ...
    let Err(error) = manager.run(&mut Module { functions: vec![leaf()] }) else { std::process::exit(1) };
...
fn default_pipeline_registers_phase_three_passes_in_order() {
    ...
    let report = manager.run(&mut Module { functions: vec![leaf()] }).fixture();
    let names = report.stats.iter().map(|stat| stat.pass.as_str()).collect::<Vec<_>>();
    assert_eq!(names, vec!["inline-direct-calls", "global-value-numbering", "sccp"]);
}
...
fn conservative_guards_and_manager_edges_are_exercised() {
    ...
    invalid.blocks[0].ops.push(Op { results: vec![(ValueId(0), Ty::Word)], kind: OpKind::Move { value: ValueId(0) }, loc: None });
    assert!(Module { functions: vec![invalid] }.verify().is_err());
    let mut zero = PassManager::with_options(PassManagerOptions { max_iterations: 0, verify_after_each_pass: true });
    let report = zero.run(&mut Module { functions: vec![leaf()] }).fixture();
    assert_eq!(report.iterations, 0);
    assert!(!report.hit_iteration_limit);
    let mut guarded = leaf();
    guarded.blocks[0].ops.push(Op { results: vec![], kind: OpKind::MakeClosure { entry: ValueId(0), captures: vec![] }, loc: None });
    ...
    assert!(!pass.run(&mut caller_function, &Module { functions: vec![leaf()] }).fixture());
    caller_function.blocks[0].ops[0].kind = OpKind::Const { result: ConstantIndex(0) };
    caller_function.constants[0] = Constant::Nil;
    assert!(!pass.run(&mut caller_function, &Module { functions: vec![leaf()] }).fixture());
    let mut non_return = leaf();
    non_return.blocks[0].terminator = Terminator::Unreachable;
    assert!(!pass.run(&mut caller(), &Module { functions: vec![non_return] }).fixture());
    let mut bad_arity = caller();
    if let OpKind::Call { args, .. } = &mut bad_arity.blocks[0].ops[1].kind { args.clear(); }
    assert!(!pass.run(&mut bad_arity, &Module { functions: vec![leaf()] }).fixture());
    let mut bad_return = leaf();
    bad_return.blocks[0].terminator = Terminator::Return { values: vec![ValueId(99)] };
    assert!(!pass.run(&mut caller(), &Module { functions: vec![bad_return] }).fixture());
}
