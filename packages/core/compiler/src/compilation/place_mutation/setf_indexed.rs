...
impl CompileState {
    pub(super) fn compile_setf_fill_pointer_place(
        ...
        if items.len() != 2
            || !Self::symbol_name_info(&items[0], "setf fill-pointer operator")
                .ok()
                .is_some_and(|(name, _)| name == "FILL-POINTER")
        {
            return Ok(false);
        }
        ...
        if let Some((name, escaped)) =
            Self::symbol_name_info(&items[1], "setf fill-pointer target").ok()
        {
            self.emit(
                function,
    ...
    pub(super) fn compile_setf_bitfield_place(
        ...
        let instruction = if let Some((name, escaped)) =
            Self::symbol_name_info(&items[2], "setf bitfield target").ok()
        {
            Instruction::SetfBitfieldDynamic {
                operator,
                name,
                escaped,
            }
