...
impl CompileState {
    pub(super) fn compile_setf_symbol_plist_place(
        &mut self,
        function: FunctionId,
        place: &Form,
        value_form: &Form,
    ...
    pub(super) fn compile_setf_getf_place(
        &mut self,
        function: FunctionId,
        place: &Form,
        value_form: &Form,
        ...
        let Ok((name, escaped)) = Self::symbol_name_info(&items[1], "setf getf target") else {
            return Ok(false);
        };
        self.compile_expression(function, &items[1])?;
