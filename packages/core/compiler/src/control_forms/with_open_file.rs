...
impl CompileState {
    ...
    fn compile_with_open(
        ...
        let binding_form = items.get(1).ok_or_else(|| {
            Self::internal_error(
                span,
                &format!("missing {operator} binding after arity check"),
            )
        })?;
        ...
        for binding in binding {
            ...
            if binding.len() < 2 {
                return Err(CompileError::new(
                    CompileErrorKind::InvalidForm {
                        message: format!(
                            "{operator} binding needs a stream variable and stream form"
                        ),
                    },
                    binding_form.span,
                ));
        ...
        let binding = Form::list(
            vec![
                Form::list(
                    vec![
                        Form::atom("S", span),
                        Form::new(FormKind::String("a.txt".to_string()), span),
                    ],
                    span,
                ),
                Form::list(
                    vec![
                        Form::atom("U", span),
                        Form::new(FormKind::String("b.txt".to_string()), span),
                    ],
                    span,
                ),
            ],
