    ...
    pub(super) fn compile_native_rotate_shift(
        ...
        if place_forms.len() == 1 {
                ...
                crate::dynamic_nth_list_place(&place_forms[0])
            {
                ...
                self.emit(function, instruction, items[0].span)?;
                return Ok(Some(()));
            }
        }
        let mixed_dynamic = place_forms
            ...
            .map(|place| {
                if let Some((index, target, accessors, name, escaped)) =
                    crate::dynamic_nth_list_place(place)
                {
                    Ok((
                        crate::RotateShiftPlace::DynamicNth(accessors, name, escaped),
                        Some((index, target)),
                    ))
        ...
        if let Ok(mixed_dynamic) = mixed_dynamic {
            if mixed_dynamic.iter().any(|(_, operands)| operands.is_some()) {
                for (place, operands) in &mixed_dynamic {
                    if let Some((index, target)) = operands {
                        ...
                        let (name, escaped) = match place {
                            ...
                            crate::RotateShiftPlace::NestedList(_, name, escaped) => {
                                (name, *escaped)
                            }
                ...
                self.emit(
                    function,
                    if operator == "ROTATEF" {
                        Instruction::RotatefDynamicMixed(places)
                    } else {
                        Instruction::ShiftfDynamicMixed(places)
                    },
                    items[0].span,
                )?;
                return Ok(Some(()));
