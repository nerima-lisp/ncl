...
#[test]
fn compile_remf_symbol_place_uses_native_instruction() {
    let mut state = CompileState::default();
    let function = state.reserve_function(None, Vec::new());
    let items = parse_items("(remf plist :key)");

    state
        .compile_runtime_definition(function, Span::new(0, 1), &items)
        .expect("REMF should compile");

    assert!(matches!(
        state.functions[function].instructions.last(),
        Some(Instruction::Remf { name, escaped: false }) if name == "PLIST"
    ));
}
...
#[test]
fn compile_psetf_uses_setf_native_instruction_for_one_generalized_place() {
    let mut state = CompileState::default();
    let function = state.reserve_function(None, Vec::new());
    let items = parse_items("(psetf (car xs) 1)");
...
fn compile_psetf_uses_native_instruction_for_symbol_plist_places() {
    ...
    state
        .compile_psetf(function, Span::new(0, 1), &items)
        .expect("PSETF SYMBOL-PLIST places should compile");

    assert!(matches!(
        state.functions[function].instructions.last(),
        Some(Instruction::PsetfPlaces(places))
            if matches!(places.as_slice(), [crate::PsetfPlace::SymbolPlist, crate::PsetfPlace::SymbolPlist])
    ));
}

#[test]
fn compile_psetf_uses_native_instruction_for_get_places() {
    let mut state = CompileState::default();
    let function = state.reserve_function(None, Vec::new());
    let items = parse_items("(psetf (get symbol :plist) 1 other 2)");

    state
        .compile_psetf(function, Span::new(0, 1), &items)
        .expect("PSETF GET places should compile");

    assert!(matches!(
        state.functions[function].instructions.last(),
        Some(Instruction::PsetfPlaces(places))
            if matches!(places.as_slice(), [crate::PsetfPlace::Get, crate::PsetfPlace::Symbol(name, false)] if name == "OTHER")
    ));
}
...
fn compile_runtime_definition_uses_native_dynamic_nth_rotate_and_shift() {
    ...
    for source in ["(rotatef (nth index xs))", "(shiftf (nth index xs) 9)"] {
        ...
        state
            .compile_runtime_definition(function, Span::new(0, 1), &items)
            .expect("dynamic NTH rotate/shift should compile");
        assert!(
            state.functions[function]
                .instructions
                .iter()
...
fn compile_runtime_definition_uses_native_dynamic_nth_mixed_rotate_and_shift() {
    let mut state = CompileState::default();
    for source in [
        "(rotatef (nth index xs) (car ys))",
        "(shiftf (car ys) (nth index xs) 9)",
    ] {
        let function = state.reserve_function(None, Vec::new());
        let items = parse_items(source);
        state
            .compile_runtime_definition(function, Span::new(0, 1), &items)
            .expect("mixed dynamic NTH rotate/shift should compile");
