...
pub(super) struct CompileState {
    pub(super) functions: Vec<FunctionCode>,
...
impl CompileState {
    ...
    pub(super) fn emit(
        ...
        let Some(code) = self.functions.get_mut(function) else {
            return Err(Self::internal_error(
                span,
                "invalid function id while emitting bytecode",
            ));
        };
    ...
    pub(super) fn patch_jump(
        ...
        let Some(code) = self.functions.get_mut(function) else {
            return Err(Self::internal_error(
                span,
                "invalid function id while patching jump",
            ));
        };
        let Some(operation) = code.instructions.get_mut(instruction) else {
            return Err(Self::internal_error(
                span,
                "invalid jump instruction position",
            ));
        };
        match operation {
