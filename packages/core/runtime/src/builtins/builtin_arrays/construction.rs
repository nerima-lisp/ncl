...
pub fn fill_pointer(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let pointer = arguments[0]
        ...
        .flatten()
        .ok_or_else(|| {
            crate::builtins::type_error("fill-pointer", "vector with a fill pointer", &arguments[0])
        })?;
            ...
            length =
                length
                    ...
                    .ok_or_else(|| crate::RuntimeError::InvalidForm {
                        message: "vector-push-extend length overflow".to_string(),
                        span: None,
                    })?;
...
pub fn make_array(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    for pair in arguments[1..].as_chunks::<2>().0 {
        ...
        match name.as_str() {
            ...
            "DISPLACED-INDEX-OFFSET" => {
                displaced_index_offset = index_argument("make-array", &pair[1])?
            }
            _ => {
                return Err(RuntimeError::InvalidForm {
    ...
    if let Some((storage, target_offset, _)) = displaced_storage.as_ref() {
        let end = target_offset
            ...
            .ok_or_else(|| crate::RuntimeError::InvalidForm {
                message: "make-array displacement overflow".to_string(),
                span: None,
            })?;
        if storage.borrow().len() < end {
            return Err(crate::RuntimeError::InvalidForm {
                message: "make-array displacement exceeds target".to_string(),
                span: None,
            });
        }
    }
    ...
    if dimensions.len() == 1 {
        let vector = if let Some((storage, target_offset, _)) = displaced_storage {
            Value::Vector(std::rc::Rc::new(crate::value::VectorData {
                elements: std::rc::Rc::new(std::cell::RefCell::new(elements)),
                metadata: std::cell::RefCell::new(crate::value::ArrayMetadata {
                    element_type: element_type.clone(),
                    adjustable,
                    fill_pointer: None,
                    displaced_to: Some(storage),
                    displaced_to_value: displaced_to.clone(),
                    displaced_index_offset: target_offset + displaced_index_offset,
                }),
            }))
        } else {
...
pub fn adjust_array(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    for pair in arguments[2..].as_chunks::<2>().0 {
        ...
        match name.as_str() {
            "INITIAL-ELEMENT" if initial_contents.is_none() => {
                initial_element = Some(pair[1].clone())
            }
            "INITIAL-CONTENTS" if initial_element.is_none() => {
                initial_contents = Some(pair[1].clone())
            }
            "INITIAL-ELEMENT" | "INITIAL-CONTENTS" => {
                return Err(crate::RuntimeError::InvalidForm {
                    message: "adjust-array cannot combine :initial-element and :initial-contents"
                        .to_string(),
                    span: None,
                })
            }
    ...
    if dimensions.len() == 1 {
        ...
        let adjustable_value =
            adjustable.unwrap_or(arguments[0].vector_adjustable().unwrap_or(false));
        if arguments[0].vector_adjustable() == Some(true) && adjustable != Some(false) {
            let fill_pointer =
                fill_pointer.or_else(|| arguments[0].vector_fill_pointer().flatten());
            if let Value::Vector(items) = &arguments[0] {
                if arguments[0].is_displaced() {
                    let (target, offset) = arguments[0]
                        .array_displacement()
                        .and_then(|displacement| displacement)
                        .ok_or_else(|| crate::RuntimeError::InvalidForm {
                            message: "adjust-array requires vector storage".to_string(),
                            span: None,
                        })?;
                    let storage_len = target
                        .array_storage()
                    ...
                    if offset + total_size > storage_len {
                        return Err(crate::RuntimeError::InvalidForm {
                            message: "adjust-array exceeds displaced array storage".to_string(),
                            span: None,
                        });
                    }
        ...
        vector.set_vector_fill_pointer(
            fill_pointer
                .or_else(|| arguments[0].vector_fill_pointer().flatten())
                .map(|value| value.min(total_size)),
        );
        Ok(vector)
    } else {
        let array = Value::array(dimensions, elements);
