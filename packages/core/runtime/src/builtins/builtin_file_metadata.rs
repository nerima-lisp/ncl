...
pub(super) fn probe_file(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    match std::fs::metadata(&path) {
        ...
        Err(error) => Err(RuntimeError::Io {
            kind: error.kind(),
            message: format!("probe-file {}: {error}", path.display()),
        }),
    }
}
...
pub(super) fn delete_file(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    std::fs::remove_file(&path).map_err(|error| RuntimeError::Io {
        kind: error.kind(),
        message: format!("delete-file {}: {error}", path.display()),
    })?;
...
pub(super) fn rename_file(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let old_truename = std::fs::canonicalize(&old_path).map_err(|error| RuntimeError::Io {
        kind: error.kind(),
        message: format!("rename-file {}: {error}", old_path.display()),
    })?;
    std::fs::rename(&old_path, &new_path).map_err(|error| RuntimeError::Io {
        kind: error.kind(),
        message: format!(
    ...
    let new_truename = std::fs::canonicalize(&new_path).map_err(|error| RuntimeError::Io {
        kind: error.kind(),
        message: format!("rename-file {}: {error}", new_path.display()),
    })?;
    Ok(Value::values(vec![
        arguments[1].clone(),
...
pub(super) fn file_write_date(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let modified = std::fs::metadata(&path)
        ...
        .map_err(|error| RuntimeError::Io {
            kind: error.kind(),
            message: format!("file-write-date {}: {error}", path.display()),
        })?;
    ...
    let seconds_since_unix =
        i64::try_from(seconds_since_unix.as_secs()).map_err(|_| RuntimeError::Io {
            kind: std::io::ErrorKind::Other,
            message: format!(
...
pub(super) fn truename(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let canonical = std::fs::canonicalize(&path).map_err(|error| RuntimeError::Io {
        kind: error.kind(),
        message: format!("truename {}: {error}", path.display()),
    })?;
