...
mod tests {
    ...
    fn bitfield_operations_follow_common_lisp_layout() {
        ...
        assert!(
            ldb(&[byte.clone(), Value::Integer(0xabc)])
                .unwrap()
                .equal_value(&Value::Integer(0xb))
        );
        assert!(
            mask_field(&[byte.clone(), Value::Integer(0xabc)])
                .unwrap()
                .equal_value(&Value::Integer(0xb0))
        );
        assert!(
            dpb(&[Value::Integer(2), byte.clone(), Value::Integer(0xabc)])
                .unwrap()
                .equal_value(&Value::Integer(0xa2c))
        );
        assert!(
            deposit_field(&[Value::Integer(0x050), byte, Value::Integer(0xabc)])
                .unwrap()
                .equal_value(&Value::Integer(0xa5c))
        );
    }
}
