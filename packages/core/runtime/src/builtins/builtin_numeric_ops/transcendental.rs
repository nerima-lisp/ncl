...
pub fn tangent(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let (real, imag, complex) = match &arguments[0] {
        Value::Complex(value) => (
            ...
            number_argument("tan", value.imaginary())?.as_float(),
            true,
        ),
...
fn unary_complex(
    ...
    match &arguments[0] {
        Value::Complex(value) => {
            ...
            let imag = number_argument(function, value.imaginary())?.as_float();
            let (real, imag) = operation(real, imag);
            Ok(Value::complex(Value::Float(real), Value::Float(imag)))
        }
        value => Ok(Value::Float(
            operation(number_argument(function, value)?.as_float(), 0.0).0,
        )),
    }
}
    ...
    if arguments.len() == 1 {
        ...
        return match &arguments[0] {
            Value::Complex(value) => {
                ...
                let imag = number_argument("log", value.imaginary())?.as_float();
                Ok(Value::complex(
            ...
            value => {
                let value = number_argument("log", value)?.as_float();
                if value.is_sign_negative() && value != 0.0 {
                    Ok(Value::complex(
                        Value::Float(value.abs().ln()),
...
pub fn arc_sine(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    match complex_argument("asin", &arguments[0])? {
        Some((real, imag)) => {
            ...
            Ok(Value::complex(
                Value::Float(log_imag),
                Value::Float(-log_real),
            ))
        }
        None => Ok(Value::Float(
            number_argument("asin", &arguments[0])?.as_float().asin(),
        )),
    }
}
...
pub fn arc_cosine(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    match complex_argument("acos", &arguments[0])? {
        Some((real, imag)) => {
            ...
            let Value::Complex(value) = asin else {
                unreachable!()
            };
            Ok(Value::complex(
                Value::Float(
                    std::f64::consts::FRAC_PI_2 - number_argument("acos", value.real())?.as_float(),
                ),
                Value::Float(-number_argument("acos", value.imaginary())?.as_float()),
            ))
        }
        None => Ok(Value::Float(
            number_argument("acos", &arguments[0])?.as_float().acos(),
        )),
    }
}
...
pub fn arc_tangent(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    match complex_argument("atan", &arguments[0])? {
        ...
        None => Ok(Value::Float(
            number_argument("atan", &arguments[0])?.as_float().atan(),
        )),
    }
}
...
fn complex_argument(function: &str, value: &Value) -> Result<Option<(f64, f64)>, RuntimeError> {
    match value {
        Value::Complex(value) => Ok(Some((
            ...
            number_argument(function, value.imaginary())?.as_float(),
        ))),
        _ => Ok(None),
    }
}
...
pub fn hyperbolic_tangent(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let (real, imag, complex) = match &arguments[0] {
        Value::Complex(value) => (
            ...
            number_argument("tanh", value.imaginary())?.as_float(),
            true,
        ),
...
mod tests {
    ...
    fn complex_unit_circle_function_accepts_real_numbers_only() {
        assert_eq!(
            cis(&[Value::Integer(0)]).unwrap().to_string(),
            "#C(1.0 0.0)"
        );
        assert_eq!(
    ...
    fn exponential_and_hyperbolic_functions_return_complex_values() {
        let value = Value::complex(Value::Integer(0), Value::Integer(1));
        assert_eq!(
            exponential(std::slice::from_ref(&value))
                .unwrap()
                .to_string(),
    ...
    fn inverse_trigonometric_functions_return_complex_principal_values() {
        ...
        assert_eq!(
            arc_cosine(std::slice::from_ref(&value))
                .unwrap()
                .to_string(),
