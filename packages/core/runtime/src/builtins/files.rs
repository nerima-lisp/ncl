...
pub(crate) fn pathname_argument(function: &str, value: &Value) -> Result<PathBuf, RuntimeError> {
    match value {
...
pub(crate) fn open_file(arguments: &[Value]) -> Result<Value, RuntimeError> {
    ...
    for pair in arguments[1..].chunks_exact(2) {
        ...
        match keyword.as_str() {
            "DIRECTION" => direction = stream_keyword_name("open :direction", &pair[1])?,
            "IF-DOES-NOT-EXIST" => {
                if_does_not_exist = Some(stream_keyword_name("open :if-does-not-exist", &pair[1])?)
            }
            "IF-EXISTS" => if_exists = Some(stream_keyword_name("open :if-exists", &pair[1])?),
...
fn open_input_file(path: &Path, if_does_not_exist: &str) -> Result<Value, RuntimeError> {
    if !path.exists() {
        match if_does_not_exist {
            "NIL" => return Ok(Value::Nil),
            "CREATE" => std::fs::write(path, [])
