...
pub(crate) fn builtin_type_specializer_score(type_name: &str) -> usize {
    let mut score: usize = 1_000;
    for supertype_name in [
        ...
        if type_name != supertype_name && builtin_subtype(type_name, supertype_name) {
...
fn builtin_subtype(subtype_name: &str, supertype_name: &str) -> bool {
    match subtype_name {
        ...
        "SERIOUS-CONDITION" => supertype_name == "CONDITION",
        "ERROR" => matches!(supertype_name, "SERIOUS-CONDITION" | "CONDITION"),
        "WARNING" => supertype_name == "CONDITION",
        "SIMPLE-CONDITION" => supertype_name == "CONDITION",
        "SIMPLE-WARNING" => matches!(supertype_name, "WARNING" | "SIMPLE-CONDITION" | "CONDITION"),
        "ARITHMETIC-ERROR" => matches!(supertype_name, "ERROR" | "SERIOUS-CONDITION" | "CONDITION"),
        "DIVISION-BY-ZERO" => matches!(
            supertype_name,
            "ARITHMETIC-ERROR" | "ERROR" | "SERIOUS-CONDITION" | "CONDITION"
        ),
        "TYPE-ERROR" => matches!(supertype_name, "ERROR" | "SERIOUS-CONDITION" | "CONDITION"),
        "SIMPLE-TYPE-ERROR" => matches!(
            supertype_name,
            "TYPE-ERROR" | "SIMPLE-CONDITION" | "ERROR" | "SERIOUS-CONDITION" | "CONDITION"
        ),
        "SIMPLE-ERROR" => matches!(
            supertype_name,
            "ERROR" | "SERIOUS-CONDITION" | "CONDITION" | "SIMPLE-CONDITION"
        ),
        "PROGRAM-ERROR" | "PACKAGE-ERROR" | "READER-ERROR" | "FILE-ERROR" | "UNBOUND-VARIABLE"
        ...
        "FUNCTION" | "STREAM" | "RANDOM-STATE" | "PACKAGE" | "ENVIRONMENT" | "HASH-TABLE" | "CONDITION"
...
fn vector_type_matches(
    ...
    let expected_size = arguments
        ...
        .transpose()?
        .flatten();
    let Some(items) = vector_elements(value) else {
        return Ok(false);
    };
    ...
    if let Some(element_type) = arguments.first() {
        if let Some(actual_element_type) = declared_array_element_type(value) {
            return declared_array_element_type_matches(
                function,
                actual_element_type,
                element_type,
            );
...
fn simple_vector_type_matches(
    ...
    let expected_size = arguments
        ...
        .transpose()?
        .flatten();
    if !is_simple_vector_value(value) {
        return Ok(false);
    }
    let Some(items) = vector_elements(value) else {
        return Ok(false);
...
fn is_bit_vector_value(value: &Value) -> bool {
    let is_declared_bit_vector = matches!(
        value,
        Value::Array {
            dimensions,
    ...
    is_declared_bit_vector
        && vector_elements(value).is_some_and(|items| items.iter().all(is_bit_value))
}

pub(super) fn is_simple_vector_value(value: &Value) -> bool {
    value.is_simple_vector()
}
...
fn array_type_matches(
    ...
    if let Some(element_type) = arguments.first() {
        if let Some(actual_element_type) = declared_array_element_type(value) {
            if !declared_array_element_type_matches(function, actual_element_type, element_type)? {
                return Ok(false);
            }
        } else if let Some(elements) = array_elements(value) {
...
pub(super) fn vector_elements(value: &Value) -> Option<Vec<Value>> {
    match value {
...
fn declared_array_element_type(value: &Value) -> Option<&'static str> {
    if matches!(value, Value::String(_) | Value::Array { .. }) || value.is_typed_vector() {
        value
            .array_element_type()
...
fn declared_array_element_type_matches(
    function: &str,
    actual: &str,
    expected: &Value,
) -> Result<bool, RuntimeError> {
    ...
    match actual {
        "T" => match expected {
            Value::List(items) => {
                ...
                let operator = type_designator_name(function, operator)?;
                let arguments = &items[1..];
                match operator.as_str() {
                    "OR" => arguments.iter().try_fold(false, |matched, argument| {
                        if matched {
                            Ok(true)
                        } else {
                            declared_array_element_type_matches(function, actual, argument)
                        }
                    }),
                    "AND" => arguments.iter().try_fold(true, |matched, argument| {
                        if !matched {
                            Ok(false)
                        } else {
                            declared_array_element_type_matches(function, actual, argument)
            ...
            _ => {
                let expected = type_designator_name(function, expected)?;
                Ok(matches!(expected.as_str(), "T" | "OBJECT"))
            }
        },
        "CHARACTER" => match expected {
            Value::List(items) => {
                let Some(operator) = items.first() else {
                    return Err(invalid_type_spec(
                        function,
...
fn type_matches(value: &Value, type_name: &str) -> Result<bool, RuntimeError> {
    let result = match type_name {
        ...
        "LIST" => {
            matches!(value, Value::Nil | Value::Boolean(false) | Value::List(_))
                || value.is_typed_list()
        }
        "ATOM" => {
            !matches!(value, Value::List(_) | Value::DottedList { .. }) && !value.is_typed_list()
        }
        "VECTOR" => vector_elements(value).is_some(),
        "SIMPLE-VECTOR" => is_simple_vector_value(value),
        "BIT-VECTOR" => is_bit_vector_value(value),
        "SIMPLE-BIT-VECTOR" => is_bit_vector_value(value),
        ...
        | "SIMPLE-TYPE-ERROR" | "PROGRAM-ERROR" | "PACKAGE-ERROR" | "READER-ERROR"
        | "COMPILER-ERROR" | "FILE-ERROR" | "UNBOUND-VARIABLE" | "CONTROL-ERROR" => {
        ...
        _ if value.condition_is_type(type_name) => true,
        _ if value.instance_is_type(type_name) => true,
        _ if value.structure_is_type(type_name) => value.structure_typep_is_type(type_name),
        _ => {
