...
pub fn typep_value_in(
    ...
    if let Ok(type_name) =
        crate::builtins::types::type_designator::type_designator_name("typep", type_designator)
    {
        if environment.lookup_condition(&type_name).is_some() {
            return Ok(value.condition_is_type(&type_name));
        }
    }
    type_matching::type_matches_designator_in("typep", value, type_designator, environment)
}
