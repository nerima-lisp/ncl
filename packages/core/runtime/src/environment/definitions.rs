...
impl Environment {
    pub(crate) fn define_type_alias(
        ...
        self.0.borrow_mut().type_aliases.insert(
            key,
            TypeAliasDefinition {
                parameters,
                optional_parameters,
    ...
    pub(crate) fn lookup_type_alias(&self, name: &str) -> Option<Value> {
        ...
        definition
            .map(|definition| definition.designator)
            .or_else(|| parent.and_then(|environment| environment.lookup_type_alias(name)))
    }
    ...
    pub(crate) fn lookup_type_alias_definition(&self, name: &str) -> Option<TypeAliasDefinition> {
        ...
        definition.or_else(|| {
            parent.and_then(|environment| environment.lookup_type_alias_definition(name))
        })
    }
