...
impl Runtime {
    pub(crate) fn apply_slot_primitive_in(
        &self,
        name: &str,
        arguments: &[Value],
        ...
        if !matches!(
            ...
            return Self::apply_slot_primitive(name, arguments, span);
        }
        let using_class = name.ends_with("-USING-CLASS");
        let expected_arguments = if using_class { 3 } else { 2 };
        if arguments.len() != expected_arguments {
            return Self::apply_slot_primitive(name, arguments, span);
        }
        let (object, slot_argument) = if using_class {
            (&arguments[1], &arguments[2])
        } else {
            (&arguments[0], &arguments[1])
        };
        let Ok(slot_name) = Self::slot_name_from_value(slot_argument, span) else {
            return Self::apply_slot_primitive(name, arguments, span);
        };
        let Value::Instance(_) = object else {
            return Self::apply_slot_primitive(name, arguments, span);
        };
        let Some(class) = object.instance_class_definition() else {
            return Self::apply_slot_primitive(name, arguments, span);
        };
        let hook_arguments = |operation: Option<&str>| {
            let mut values = vec![
                Value::class_object(class.clone()),
                object.clone(),
                Value::symbol(slot_name.clone()),
            ];
            if let Some(operation) = operation {
                values.push(Value::symbol(operation));
            }
            values
        };
        let apply_hook = |hook: &str, hook_arguments: Vec<Value>| {
            let function = environment
                .lookup_function(hook)
                .unwrap_or_else(|| match hook {
                    "SLOT-MISSING" => Value::primitive("SLOT-MISSING"),
                    "SLOT-UNBOUND" => Value::primitive("SLOT-UNBOUND"),
                    _ => unreachable!("slot hook was prevalidated"),
                });
            self.apply_in(&function, &hook_arguments, span, environment)
        };
        match name {
            "SLOT-VALUE" | "SLOT-VALUE-USING-CLASS" => match object.instance_slot(&slot_name) {
                None => Some(apply_hook(
                    "SLOT-MISSING",
                    hook_arguments(Some("SLOT-VALUE")),
                )),
                Some(Value::Unbound) => Some(apply_hook("SLOT-UNBOUND", hook_arguments(None))),
                Some(_) => Self::apply_slot_primitive(name, arguments, span),
            },
                ...
                if !object.instance_slot_exists(&slot_name) =>
            {
                Some(apply_hook(
                    "SLOT-MISSING",
                    hook_arguments(Some("SLOT-MAKUNBOUND")),
                ))
            }
            _ => Self::apply_slot_primitive(name, arguments, span),
        }
    }
    ...
    pub(crate) fn slot_definition_value(slot: &ClassSlot) -> Value {
        ...
        Value::instance(
            class.clone(),
            vec![
                ("CLASS".to_owned(), Value::class_object(class)),
                (
                    "NAME".to_owned(),
