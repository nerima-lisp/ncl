        ...
        if name == "RENAME-PACKAGE" {
            return Some((|| -> Result<Value, RuntimeError> {
                ...
                let nicknames = arguments
                    .get(2)
                    .map(|value| {
                        let values = value.list_items().unwrap_or_else(|| vec![value.clone()]);
                        values
                            .into_iter()
                ...
                let renamed = self
                    ...
                    .map_err(|message| Self::package_error(&message, span))?;
                self.rename_package_prefix(&old_name, &renamed);
                self.packages
                    .borrow()
                    .package_object_for(&renamed)
            ...
            while index < arguments.len() {
                let keyword = arguments[index]
                    ...
                    .ok_or_else(|| RuntimeError::Type {
                        ...
                        actual: arguments[index].type_name().into(),
                        span: Some(span),
                    })?;
                let value = arguments
                    .get(index + 1)
                    .ok_or_else(|| Self::arity(name, "keyword/value pairs", arguments.len()))?;
                let values = value.list_items().unwrap_or_else(|| vec![value.clone()]);
                match keyword
                    .trim_start_matches(':')
                    .to_ascii_uppercase()
                    .as_str()
                {
                    ...
                    "USE" => {
                        use_packages = values
                            .into_iter()
                            .map(|value| Self::package_name_from_value(&self, &value, span))
                            .collect::<Result<_, _>>()?
                    }
                    _ => {
                        return Err(Self::package_error(
                            &format!("unknown MAKE-PACKAGE keyword {keyword}"),
                            span,
                        ));
                    }
                }
