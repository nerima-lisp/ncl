...
impl Runtime {
    pub(crate) fn special_define_condition(
        ...
        for parent in parents {
            ...
            if !matches!(
                parent.as_str(),
                "CONDITION" | "SERIOUS-CONDITION" | "WARNING" | "ERROR"
            ) && environment.lookup_condition(&parent).is_none()
            {
                return Err(Self::invalid(
                    "unknown define-condition parent",
                    items[2].span,
                ));
            }
        ...
        for slot in slots {
            ...
            let slot_name = unqualified_name(&Self::variable_name(
                slot_name,
                "define-condition slot must be a symbol",
            )?);
            ...
            for pair in options.as_chunks::<2>().0 {
                let option =
                    Self::definition_name_from_form(&pair[0], "define-condition slot option")?;
                match option.as_str() {
                    "READER" | "WRITER" | "ACCESSOR" => {
                        let accessor = unqualified_name(&Self::variable_name(
                            &pair[1],
                            "condition accessor must be a symbol",
                        )?);
                        if option == "READER" || option == "ACCESSOR" {
                            environment.define_function(
                                &accessor,
                                Value::condition_reader(name.clone(), slot_name.clone()),
                            );
                        }
                        if option == "WRITER" {
                            environment.define_function(
                                &accessor,
                                Value::condition_writer(name.clone(), slot_name.clone()),
                            );
                        }
                    }
                    "INITARG" => {
                        let initarg =
                            Self::definition_name_from_form(&pair[1], "condition initarg")?;
                    ...
                    _ => {
                        return Err(Self::invalid(
                            "unsupported define-condition slot option",
                            pair[0].span,
                        ));
                    }
                }
        ...
        for option in items.iter().skip(4) {
            ...
            if values.is_empty() {
                return Err(Self::invalid(
                    "define-condition option must be non-empty",
                    option.span,
                ));
            }
            let option_name =
                Self::definition_name_from_form(&values[0], "define-condition option name")?;
            if option_name == "DOCUMENTATION"
                && (values.len() != 2 || !matches!(values[1].kind, FormKind::String(_)))
            {
                return Err(Self::invalid(
        ...
        environment.define_condition(
            name.clone(),
            crate::environment::ConditionDefinition {
