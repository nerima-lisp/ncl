...
impl Runtime {
    pub(crate) fn special_deftype(
        ...
        let arguments = match &items[2].kind {
            ...
            _ => {
                return Err(Self::invalid(
                    "deftype lambda list must be a proper list",
                    items[2].span,
                ));
            }
        };
        ...
        for argument in arguments {
            ...
            let (parameter, _) =
                Self::variable_name_info(argument, "deftype parameters must be symbols")?;
        ...
        if marker < arguments.len() {
            if atom_name(&arguments[marker]).is_some_and(|name| name.eq_ignore_ascii_case("&rest"))
            {
                if arguments.len() != marker + 2 {
                    return Err(Self::invalid(
                        "deftype &rest must be followed by one parameter",
                        arguments[marker].span,
                    ));
                }
                let (parameter, _) =
                    Self::variable_name_info(&arguments[marker + 1], "deftype rest parameter")?;
                rest_parameter = Some(crate::environment::intern_name(&unqualified_name(
                    &parameter,
                )));
            } else if !atom_name(&arguments[marker])
                .is_some_and(|name| name.eq_ignore_ascii_case("&optional"))
            {
                return Err(Self::invalid(
                    "invalid deftype lambda list",
                    arguments[marker].span,
                ));
            } else {
                for argument in arguments.iter().skip(marker + 1) {
                    let (parameter, default) = match &argument.kind {
                        FormKind::Atom(_) => (
                            Self::variable_name_info(argument, "deftype parameter")?.0,
                            Value::Nil,
                        ),
                        ...
                        _ => {
                            return Err(Self::invalid(
                                "invalid deftype optional parameter",
                                argument.span,
                            ));
                        }
                    };
                    optional_parameters.push((
                        crate::environment::intern_name(&unqualified_name(&parameter)),
                        default,
                    ));
                }
            }
