...
impl Runtime {
    pub(super) fn condition_format_control(value: &Value) -> Option<String> {
        match value {
    ...
    pub(super) fn condition_message(
        value: &Value,
        arguments: &[Value],
        span: Span,
    ...
    pub(super) fn signaled_error(
        ...
        RuntimeError::Signaled(Box::new(crate::error::SignaledError {
...
impl Runtime {
    ...
    pub(super) fn make_condition(
        ...
        for pair in initargs.chunks_exact(2) {
            ...
            match initarg.as_str() {
                ...
                _ => {
                    return Err(
                        self.invalid(&format!("unknown make-condition initarg :{initarg}"), span)
                    );
                }
            }
        }
    ...
    pub(super) fn signal_condition_value(
        &self,
        condition: &Value,
        warning: bool,
        environment: &Environment,
    ...
    pub(super) fn signal_condition(&self, request: SignalRequest<'_>) -> Result<(), RuntimeError> {
        let error = Self::signaled_error(
            request.condition,
            Vec::new(),
            request.message,
            request.format_control,
            request.format_arguments,
            request.warning,
            request.span,
        );
        let condition_value = Value::condition(&error);
        self.dispatch_condition(error, &condition_value, request.environment, request.span)
    }
    ...
    pub(super) fn restart_binding_for_designator_in(
        &self,
        designator: &Value,
    ...
    pub(super) fn restart_binding_for_designator(
        &self,
        designator: &Value,
        span: Span,
    ) -> Result<Option<RestartBinding>, RuntimeError> {
    ...
    pub(super) fn invoke_restart_binding(
        &self,
    ...
    pub(super) fn invoke_restart_named(
        &self,
        name: &str,
        arguments: &[Value],
        environment: &Environment,
