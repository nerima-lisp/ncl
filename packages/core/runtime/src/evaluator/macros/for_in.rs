...
use crate::{Runtime, RuntimeError, environment::names_equal, evaluator::helpers::atom_name};

use super::loop_aggregate::{append_step, count_step, sum_step};
use super::loop_hash::bind_hash_value_and_key;

pub(super) fn expand_loop_for_in(
    form: &Form,
    items: &[Form],
    ...
    let mut collect_name = Form::atom(format!("NCL-LOOP-COLLECT-{}", form.span.start), form.span);
    if items.len() < 5 {
    ...
    if let Some(condition_name) = items.get(body_start).and_then(atom_name) {
            ...
            || names_equal(condition_name, "NEVER")
        {
            ...
            loop_condition = Some((condition_name.to_owned(), items[body_start + 1].clone()));
            body_start += 2;
        }
    }
    ...
    if let (Some(value), Some(name)) = (extremum_form, extremum_name.clone()) {
        ...
        let candidate = Form::atom(format!("NCL-LOOP-CANDIDATE-{}", form.span.start), form.span);
            ...
            vec![
                ...
                Form::list(
                    vec![
                        ...
                        Form::list(vec![Form::atom("NULL", form.span), name.clone()], form.span),
                        Form::list(
                            vec![
