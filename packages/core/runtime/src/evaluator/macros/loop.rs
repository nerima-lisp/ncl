    ...
    pub(super) fn expand_builtin_loop(form: &Form) -> Result<Form, RuntimeError> {
        ...
        if let Some(condition_body) = expand_loop_condition(form, items)? {
            ...
            if names_equal(clause, "REPEAT") {
                ...
                if let Some(across) = parse_across_clause(form, items)? {
                    ...
                    let vector_loop = Form::list(
                        vec![
                            Form::atom("LET", form.span),
                            Form::list(
                                vec![Form::list(vec![vector.clone(), across.vector], form.span)],
                                form.span,
                            ),
                            dotimes,
                        ],
                ...
                if clause_condition.is_none() {
                    clause_condition = parse_loop_clause_condition(form, items, &mut body_start)?;
                }
                if items
                    .get(body_start)
                    .and_then(atom_name)
                ...
                do_items.push(Form::list(termination, form.span));
                if let Some(value) = collect_form.clone() {
                    do_items.push(Form::list(
                        vec![Form::atom("PUSH", form.span), value, collect_name.clone()],
                        form.span,
