...
pub(super) fn expand_loop_hash_being(
        ...
        .is_some_and(|name| names_equal(name, "USING"))
    {
        ...
        if names_equal(kind, "HASH-VALUES") {
            ...
            return Ok(Some(Runtime::expand_builtin_loop(&Form::list(
                rewritten, form.span,
            ))?));
        }
    ...
    Ok(Some(Runtime::expand_builtin_loop(&Form::list(
        rewritten, form.span,
    ))?))
}
                ...
                vec![
                    Form::list(
                        vec![
                            value_binding.clone(),
                            Form::list(
                                vec![Form::atom("GETHASH", form.span), key.clone(), table.clone()],
                                form.span,
                            ),
                        ],
                        form.span,
