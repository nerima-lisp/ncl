...
impl Runtime {
    pub(super) fn expand_builtin_all_symbol_iteration(form: &Form) -> Result<Form, RuntimeError> {
        let FormKind::List(items) = &form.kind else {
            return Ok(form.clone());
        };
        if items.len() < 2 {
            return Err(Self::arity("DO-ALL-SYMBOLS", "at least one", 0));
        }
        let FormKind::List(binding) = &items[1].kind else {
            return Err(Self::invalid("symbol iteration binding must be a list", items[1].span));
        };
        if !(1..=2).contains(&binding.len()) || atom_name(&binding[0]).is_none() {
            return Err(Self::invalid("DO-ALL-SYMBOLS binding is invalid", items[1].span));
        }
        let result = binding
            .get(1)
            .cloned()
            .unwrap_or_else(|| Form::atom("NIL", form.span));
        let symbols = Form::list(vec![Form::atom("__NCL-ALL-SYMBOLS", form.span)], form.span);
        let mut dolist = vec![
            Form::atom("DOLIST", form.span),
            Form::list(vec![binding[0].clone(), symbols, result], items[1].span),
        ];
