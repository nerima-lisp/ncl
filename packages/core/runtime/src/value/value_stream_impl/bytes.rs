...
impl Stream {
    pub(in crate::value) fn file_byte_input(bytes: Vec<u8>) -> Self {
        Self {
            kind: super::super::value_stream::StreamKind::Input {
    ...
    pub(in crate::value) fn file_byte_output(path: PathBuf, bytes: Vec<u8>) -> Self {
        ...
        Self {
            kind: super::super::value_stream::StreamKind::Output {
                buffer: String::new(),
                position: 0,
                destination: None,
                at_line_start: true,
    ...
    pub(in crate::value) fn file_byte_io(path: PathBuf, bytes: Vec<u8>, append: bool) -> Self {
        ...
        Self {
            kind: super::super::value_stream::StreamKind::Io {
                characters: Vec::new(),
                position: 0,
                pushback: None,
                at_line_start: true,
    ...
    pub(crate) fn write_byte(&mut self, byte: u8) -> bool {
        let Some(data) = self.byte_data.as_mut() else {
            return false;
        };
        match data {
            ByteStreamData::Output {
                bytes, position, ..
            } => {
