...
pub fn execute_stream_operation_instruction(
    ...
    let standard_input = runtime
        .lookup_in("*standard-input*", environment)
        .and_then(|value| match value {
            Value::Stream(stream) => Some(stream),
            _ => None,
        });
    let standard_output = runtime
        .lookup_in("*standard-output*", environment)
        .and_then(|value| match value {
            Value::Stream(stream) => Some(stream),
            _ => None,
        });
    let result =
        crate::builtins::with_stream_context(
            standard_input,
            standard_output,
            || match operation {
                ...
                "MAKE-STRING-OUTPUT-STREAM" => {
                    crate::builtins::make_string_output_stream(&arguments)
                }
                ...
                "READ-PRESERVING-WHITESPACE" => {
                    crate::builtins::read_preserving_whitespace(&arguments)
                }
