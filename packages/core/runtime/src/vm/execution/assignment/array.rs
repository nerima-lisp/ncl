...
pub(super) fn execute(
    ...
    match instruction {
        ...
        Instruction::ArrayMutationPushNewOptions { rank, accessor, name, escaped, test_not, has_key, key_before_test } => execute_array_pushnew_options(
            runtime, *rank, accessor, name, *escaped, *test_not, *has_key, *key_before_test,
            stack, environment, program_counter, span,
        ),
        _ => Ok(false),
    }
}
...
fn execute_array_pushnew_options(
    runtime: &Runtime, rank: usize, accessor: &str, name: &str, escaped: bool,
    test_not: bool, has_key: bool, key_before_test: bool, stack: &mut Vec<Value>,
    ...
    let target = stack.pop().ok_or_else(|| invalid("array pushnew has no target", span))?.primary_value();
    let value = stack.pop().ok_or_else(|| invalid("array pushnew has no value", span))?.primary_value();
    let (test, key) = if key_before_test {
        let test = stack.pop().ok_or_else(|| invalid("array pushnew has no test", span))?.primary_value();
        let key = stack.pop().ok_or_else(|| invalid("array pushnew has no key", span))?.primary_value();
        (test, Some(key))
    } else {
        let key = if has_key { Some(stack.pop().ok_or_else(|| invalid("array pushnew has no key", span))?.primary_value()) } else { None };
        let test = stack.pop().ok_or_else(|| invalid("array pushnew has no test", span))?.primary_value();
        (test, key)
    };
    ...
    let indices = indices.iter().map(|index| crate::builtins::index_argument("array pushnew", index)).collect::<Result<Vec<_>, _>>()?;
    ...
    let item_key = key.as_ref().map_or_else(|| Ok(value.clone()), |key| runtime.apply_in(key, std::slice::from_ref(&value), span, environment).map(|v| v.primary_value()))?;
    let found = elements.iter().map(|candidate| {
        let candidate_key = key.as_ref().map_or_else(|| Ok(candidate.clone()), |key| runtime.apply_in(key, std::slice::from_ref(candidate), span, environment).map(|v| v.primary_value()))?;
        runtime.apply_in(&test, &[item_key.clone(), candidate_key], span, environment).map(|v| v.primary_value().is_truthy())
    }).collect::<Result<Vec<_>, _>>()?.into_iter().any(|equal| if test_not { !equal } else { equal });
    let result = if found { current } else { elements.insert(0, value); let updated = Value::list(elements); set_array_mutation_item(&target, offset, updated.clone(), span)?; updated };
...
fn array_mutation_offset(target: &Value, accessor: &str, rank: usize, indices: &[usize], span: Span) -> Result<usize, RuntimeError> {
    match target {
        ...
        Value::Array { dimensions, .. } if accessor != "SVREF" => {
            if accessor == "ROW-MAJOR-AREF" && rank == 1 {
                ...
                array_offset(dimensions, rank, indices, "array pushnew has the wrong number of indices", span)
            }
        }
        other => Err(RuntimeError::Type { expected: "ARRAY or VECTOR".to_string(), actual: other.type_name().to_string(), span: Some(span) }),
    }
}

fn array_mutation_item(target: &Value, offset: usize, span: Span) -> Result<Value, RuntimeError> {
    match target {
        Value::Vector(_) => target.vector_items().and_then(|items| items.get(offset).cloned()),
        Value::Array { .. } => target.array_items().and_then(|items| items.get(offset).cloned()),
        _ => None,
    }.ok_or_else(|| invalid("array pushnew index is out of bounds", span))
}

fn set_array_mutation_item(target: &Value, offset: usize, value: Value, span: Span) -> Result<(), RuntimeError> {
    let updated = match target {
        Value::Vector(_) => target.set_vector_item(offset, value),
        Value::Array { .. } => target.set_array_item(offset, value),
        _ => None,
    };
    updated.ok_or_else(|| invalid("array pushnew index is out of bounds", span)).map(|_| ())
}

fn execute_modify_aref(
    runtime: &Runtime, rank: usize, arithmetic: &str, operator: &str, name: &str,
    escaped: bool, stack: &mut Vec<Value>, environment: &Environment,
    program_counter: &mut usize, span: Span,
) -> Result<bool, RuntimeError> {
    let delta = stack.pop().ok_or_else(|| invalid("modify aref has no delta", span))?.primary_value();
    if stack.len() < rank + 1 { return Err(invalid("modify aref has an incomplete stack", span)); }
    let indices = stack.split_off(stack.len() - rank);
    let indices = indices.iter().map(|index| crate::builtins::index_argument("modify array accessor", index)).collect::<Result<Vec<_>, _>>()?;
    let target = stack.pop().ok_or_else(|| invalid("modify aref has no target", span))?.primary_value();
    let offset = match &target {
        Value::Vector(_) if rank == 1 => indices[0],
        Value::Array { dimensions, .. } if operator != "SVREF" => if operator == "ROW-MAJOR-AREF" && rank == 1 { indices[0] } else { array_offset(dimensions, rank, &indices, "modify array has the wrong number of indices", span)? },
        other => return Err(RuntimeError::Type { expected: "ARRAY or VECTOR".to_string(), actual: other.type_name().to_string(), span: Some(span) }),
    };
    let current = match &target { Value::Vector(_) => target.vector_items().and_then(|items| items.get(offset).cloned()), Value::Array { .. } => target.array_items().and_then(|items| items.get(offset).cloned()), _ => None }.ok_or_else(|| invalid("MODIFY array index is out of bounds", span))?;
    let value = runtime.apply_in(&Value::symbol(arithmetic.to_string()), &[current, delta], span, environment)?.primary_value();
    match &target { Value::Vector(_) => { target.set_vector_item(offset, value.clone()); }, Value::Array { .. } => { target.set_array_item(offset, value.clone()); }, _ => unreachable!() }
    store_array_value(runtime, name, escaped, target, value, stack, environment, program_counter, span)
}
