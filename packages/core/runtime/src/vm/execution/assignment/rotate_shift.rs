...
fn dynamic_nth_place_values(
    ...
    places
        ...
        .map(|((accessors, _, _), pair)| {
            let index =
                crate::builtins::index_argument("ROTATEF/SHIFTF NTH", &pair[0].primary_value())?;
...
pub(super) fn execute_rotatef_nth_dynamic_places(
    ...
    for (index, ((accessors, name, escaped), pair)) in
        places.iter().zip(values.chunks_exact(2)).enumerate()
    {
...
pub(super) fn execute_shiftf_nth_dynamic_places(
    ...
    let mut updated_roots: Vec<(&str, bool, Value)> = Vec::new();
    for (index, ((accessors, name, escaped), pair)) in places
        .iter()
        .zip(values[..places.len() * 2].chunks_exact(2))
        .enumerate()
    {
        let current_root = updated_roots
            .iter()
        ...
        let value = old
            .get(index + 1)
            .cloned()
            .unwrap_or_else(|| values.last().unwrap().clone());
        let updated = Value::list(super::list::nested::update_dynamic(
            list, accessors, nth, &value, span,
        )?);
        if *escaped {
            runtime.set_or_define_exact_in(name, updated.clone(), environment, span)?;
        } else {
...
pub(super) fn execute_rotatef_nth_dynamic(
    ...
    let root = stack
        .pop()
        .ok_or_else(|| invalid("rotatef NTH has no target", span))?;
    let index = stack
        .pop()
        .ok_or_else(|| invalid("rotatef NTH has no index", span))?;
    let index = crate::builtins::index_argument("rotatef NTH", &index.primary_value())?;
    let elements = root.list_items().ok_or_else(|| RuntimeError::Type {
        expected: "LIST".into(),
    ...
    let old = if accessors.is_empty() {
        elements
            .get(index)
            .cloned()
            .ok_or_else(|| crate::builtins::out_of_bounds("rotatef NTH", index))?
    } else {
        super::list::nested::read(elements.clone(), accessors, span)?
            .list_items()
            .ok_or_else(|| RuntimeError::Type {
                expected: "LIST".into(),
                actual: "not a list".into(),
                span: Some(span),
            })?
            .get(index)
            .cloned()
    ...
    let updated = Value::list(super::list::nested::update_dynamic(
        elements, accessors, index, &old, span,
    )?);
    if escaped {
...
pub(super) fn execute_shiftf_nth_dynamic(
    ...
    let new_value = stack
        .pop()
        .ok_or_else(|| invalid("shiftf NTH has no value", span))?;
    let root = stack
        .pop()
        .ok_or_else(|| invalid("shiftf NTH has no target", span))?;
    let index = stack
        .pop()
        .ok_or_else(|| invalid("shiftf NTH has no index", span))?;
    ...
    let old = target
        .list_items()
        .and_then(|items| items.get(index).cloned())
        .ok_or_else(|| crate::builtins::out_of_bounds("shiftf NTH", index))?;
    let updated = Value::list(super::list::nested::update_dynamic(
        elements,
        accessors,
        index,
...
pub(super) fn execute_rotatef(
    ...
    for (index, (name, escaped)) in places.iter().enumerate() {
        let value = values[(index + 1) % values.len()]
            .clone()
            .primary_value();
        if *escaped {
...
pub(super) fn execute_rotatef_dynamic_mixed(
    ...
    let count = places
        .iter()
        .map(|place| {
            if matches!(place, ncl_compiler::RotateShiftPlace::DynamicNth(_, _, _)) {
                2
            } else {
                1
            }
    ...
    for place in places {
        let (index, root) = match place {
            ncl_compiler::RotateShiftPlace::DynamicNth(_, _, _) => {
                let index = crate::builtins::index_argument(
                    "ROTATEF NTH",
                    &values[cursor].primary_value(),
                )?;
                cursor += 1;
                let root = values[cursor].clone();
                cursor += 1;
                (Some(index), root)
            }
            _ => {
        ...
        let target = match place {
            ncl_compiler::RotateShiftPlace::Symbol(_, _) => root.clone(),
            ncl_compiler::RotateShiftPlace::NestedList(accessors, _, _)
            | ncl_compiler::RotateShiftPlace::DynamicNth(accessors, _, _) => {
                let elements = root.list_items().ok_or_else(|| RuntimeError::Type {
                    expected: "LIST".into(),
                    actual: root.type_name().into(),
                    span: Some(span),
                ...
                if let Some(index) = index {
                    let list = if accessors.is_empty() {
                        Value::list(elements.clone())
                    } else {
                        super::list::nested::read(elements.clone(), accessors, span)?
                    };
                    ...
                    super::list::nested::read(elements, accessors, span)?
                }
            }
        };
        roots.push(root);
...
pub(super) fn execute_shiftf_dynamic_mixed(
    ...
    for place in places {
        let (index, root) = match place {
            ncl_compiler::RotateShiftPlace::DynamicNth(_, _, _) => {
                let i =
                    crate::builtins::index_argument("SHIFTF NTH", &values[cursor].primary_value())?;
                cursor += 1;
                let r = values[cursor].clone();
                cursor += 1;
                (Some(i), r)
            }
            _ => {
        ...
        let target = match place {
            ncl_compiler::RotateShiftPlace::Symbol(_, _) => root.clone(),
            ncl_compiler::RotateShiftPlace::NestedList(a, _, _)
            | ncl_compiler::RotateShiftPlace::DynamicNth(a, _, _) => {
                let e = root.list_items().ok_or_else(|| RuntimeError::Type {
                    expected: "LIST".into(),
                    actual: root.type_name().into(),
                    span: Some(span),
                })?;
                if let Some(i) = index {
                    let l = if a.is_empty() {
                        ...
                        super::list::nested::read(e.clone(), a, span)?
                    };
                    l.list_items()
                        .and_then(|x| x.get(i).cloned())
                        .ok_or_else(|| crate::builtins::out_of_bounds("SHIFTF NTH", i))?
                } else {
                    super::list::nested::read(e, a, span)?
                }
            }
        };
        roots.push(root);
    ...
    for (i, (place, _root)) in places.iter().zip(roots).enumerate() {
        let value = old
            ...
            .primary_value();
        match place {
            ncl_compiler::RotateShiftPlace::Symbol(n, e) => {
                if *e {
                    runtime.set_or_define_exact_in(n, value, environment, span)?
                } else {
                    runtime.set_or_define_in(n, value, environment, span)?
                }
            }
            ncl_compiler::RotateShiftPlace::NestedList(a, n, e) => {
                let current_root = if *e {
                    runtime.lookup_exact_in(n, environment)
                } else {
                    runtime.lookup_in(n, environment)
                }
                .ok_or_else(|| invalid("shiftf nested place root is unbound", span))?;
                let u = Value::list(super::list::nested::update(
                    current_root
                        .list_items()
                        .ok_or_else(|| RuntimeError::Type {
                            expected: "LIST".into(),
                ...
                if *e {
                    runtime.set_or_define_exact_in(n, u, environment, span)?
                } else {
                    runtime.set_or_define_in(n, u, environment, span)?
                }
            }
            ncl_compiler::RotateShiftPlace::DynamicNth(a, n, e) => {
                let current_root = if *e {
                    runtime.lookup_exact_in(n, environment)
                } else {
                    runtime.lookup_in(n, environment)
                ...
                .ok_or_else(|| invalid("shiftf nested place root is unbound", span))?;
                let u = Value::list(super::list::nested::update_dynamic(
                    current_root
                        .list_items()
                        .ok_or_else(|| RuntimeError::Type {
                            expected: "LIST".into(),
