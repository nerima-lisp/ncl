...
pub(super) fn execute(
    ...
    match instruction {
        ...
        Instruction::ModifyNthDynamic {
            ...
            let delta = stack
                .pop()
                .ok_or_else(|| invalid("modify nth has no delta", span))?
                .primary_value();
            let current = stack
                .pop()
                .ok_or_else(|| invalid("modify nth has no target", span))?
                .primary_value();
            let index = stack
                .pop()
                .ok_or_else(|| invalid("modify nth has no index", span))?
                .primary_value();
            ...
            let slot = elements
                .get(index)
                .ok_or_else(|| crate::builtins::out_of_bounds("modify nth", index))?
                .clone();
            let value = runtime
                .apply_in(
        ...
        Instruction::ListMutationNthDynamic {
            ...
            let current = stack
                .pop()
                .ok_or_else(|| invalid("nth mutation has no target", span))?
                .primary_value();
            let index = stack
                .pop()
                .ok_or_else(|| invalid("nth mutation has no index", span))?
                .primary_value();
            ...
            let slot = elements
                .get(index)
                .ok_or_else(|| crate::builtins::out_of_bounds("nth mutation", index))?
                .clone();
            let value = if operator == "PUSH" {
                let value = stack
                    .pop()
                    .ok_or_else(|| invalid("nth PUSH has no value", span))?
                    .primary_value();
                let mut slot = slot.list_items().ok_or_else(|| RuntimeError::Type {
                    expected: "LIST".to_string(),
                ...
                if *escaped {
                    runtime.set_or_define_exact_in(name, outer, environment, span)?;
                } else {
                    runtime.set_or_define_in(name, outer, environment, span)?;
                }
                stack.push(popped);
                *program_counter += 1;
                return Ok(true);
        ...
        Instruction::ListMutationNthPushNew { name, escaped } => {
            let current = stack
                .pop()
                .ok_or_else(|| invalid("nth PUSHNEW has no target", span))?
                .primary_value();
            let index = stack
                .pop()
                .ok_or_else(|| invalid("nth PUSHNEW has no index", span))?
                .primary_value();
            let value = stack
                .pop()
                .ok_or_else(|| invalid("nth PUSHNEW has no value", span))?
                .primary_value();
            ...
            let slot = elements
                .get(index)
                .ok_or_else(|| crate::builtins::out_of_bounds("nth PUSHNEW", index))?
                .clone();
            let mut slot = slot.list_items().ok_or_else(|| RuntimeError::Type {
                expected: "LIST".to_string(),
                actual: slot.type_name().to_string(),
        ...
        Instruction::ListMutationNthPushNewOptions {
            ...
            let (test, key) = if *key_before_test {
                let test = stack
                    .pop()
                    .ok_or_else(|| invalid("nth PUSHNEW has no test", span))?
                    .primary_value();
                let key = stack
                    .pop()
                    .ok_or_else(|| invalid("nth PUSHNEW has no key", span))?
                    .primary_value();
                (test, Some(key))
            } else {
                let key = if *has_key {
            ...
            let item_key = if let Some(key) = &key {
                runtime
            ...
            let found = slot
                .iter()
                .map(|candidate| {
                    let candidate_key = if let Some(key) = &key {
                        runtime
                    ...
                    runtime
                        ...
                        .map(|result| result.primary_value().is_truthy())
                })
        ...
        Instruction::NestedListMutationNthPushNewOptions {
            ...
            let current = stack
                .pop()
                .ok_or_else(|| invalid("nested nth PUSHNEW has no target", span))?
                .primary_value();
            let index = stack
                .pop()
                .ok_or_else(|| invalid("nested nth PUSHNEW has no index", span))?
                .primary_value();
            let value = stack
                .pop()
                .ok_or_else(|| invalid("nested nth PUSHNEW has no value", span))?
                .primary_value();
            let (test, key) = if *key_before_test {
                let test = stack
                    .pop()
                    .ok_or_else(|| invalid("nested nth PUSHNEW has no test", span))?
                    .primary_value();
                let key = stack
                    .pop()
                    .ok_or_else(|| invalid("nested nth PUSHNEW has no key", span))?
                    .primary_value();
                (test, Some(key))
            } else {
                let key = if *has_key {
            ...
            let target = crate::vm::execution::assignment::list::nested::read(
                elements.clone(),
                accessors,
                span,
            )?;
            let mut target_items = target.list_items().ok_or_else(|| RuntimeError::Type {
                expected: "LIST".to_string(),
                actual: target.type_name().to_string(),
                span: Some(span),
            })?;
            let slot = target_items
                .get(index)
                .ok_or_else(|| crate::builtins::out_of_bounds("nested nth PUSHNEW", index))?
                .clone();
            let mut slot = slot.list_items().ok_or_else(|| RuntimeError::Type {
                expected: "LIST".to_string(),
                actual: slot.type_name().to_string(),
            ...
            let result = if found {
                ...
                let updated = Value::list(crate::vm::execution::assignment::list::nested::update(
                    elements,
                    accessors,
                    &Value::list(target_items),
                    span,
                )?);
                if *escaped {
                    runtime.set_or_define_exact_in(name, updated, environment, span)?;
        ...
        Instruction::NestedListMutationNthDynamic {
            ...
            let current = stack
                .pop()
                .ok_or_else(|| invalid("nested nth mutation has no target", span))?
                .primary_value();
            let index = stack
                .pop()
                .ok_or_else(|| invalid("nested nth mutation has no index", span))?
                .primary_value();
            ...
            let value = if matches!(operator.as_str(), "PUSH" | "PUSHNEW") {
                Some(
                    stack
                        .pop()
                        .ok_or_else(|| invalid("nested nth PUSH has no value", span))?
                        .primary_value(),
                )
            } else {
                None
            ...
            let slot = target_items
                .get(index)
                .ok_or_else(|| crate::builtins::out_of_bounds("nested nth mutation", index))?
                .clone();
            let mut slot_items = slot.list_items().ok_or_else(|| RuntimeError::Type {
                expected: "LIST".to_string(),
                actual: slot.type_name().to_string(),
                span: Some(span),
            })?;
            let result = match operator.as_str() {
                "PUSH" => {
                    slot_items.insert(0, value.expect("PUSH value"));
                    Value::list(slot_items)
                }
                "PUSHNEW" => {
                    let value = value.expect("PUSHNEW value");
                    if slot_items.iter().any(|candidate| {
                        crate::builtins::type_predicates::eql_value(&value, candidate)
                    }) {
                        Value::list(slot_items)
                    } else {
                        slot_items.insert(0, value);
                        Value::list(slot_items)
                    }
                }
                "POP" => {
                    let popped = slot_items.first().cloned().unwrap_or(Value::Nil);
        ...
        Instruction::RotatefNthDynamic {
            accessors,
            name,
            escaped,
        } => rotate_shift::execute_rotatef_nth_dynamic(
            accessors,
            name,
            *escaped,
            stack,
        ...
        Instruction::ShiftfNthDynamic {
            accessors,
            name,
            escaped,
        } => rotate_shift::execute_shiftf_nth_dynamic(
            accessors,
            name,
            *escaped,
            stack,
        ...
        Instruction::RotatefNthDynamicPlaces(places) => {
            rotate_shift::execute_rotatef_nth_dynamic_places(
                places,
                stack,
                environment,
                runtime,
        ...
        Instruction::ShiftfNthDynamicPlaces(places) => {
            rotate_shift::execute_shiftf_nth_dynamic_places(
                places,
                stack,
                environment,
                runtime,
        ...
        Instruction::ShiftfDynamicMixed(places) => rotate_shift::execute_shiftf_dynamic_mixed(
            places,
            stack,
            environment,
            runtime,
        ...
        Instruction::RotatefDynamicMixed(places) => rotate_shift::execute_rotatef_dynamic_mixed(
            places,
            stack,
            environment,
            runtime,
