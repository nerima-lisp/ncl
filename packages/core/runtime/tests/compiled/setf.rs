...
#[test]
fn compiled_evaluates_pushnew_on_dynamic_nth_places() {
    assert_eq!(
        evaluate("(let ((xs (list (list 2) (list 4))) (index 1)) (list (pushnew 4 (nth index xs)) (pushnew 3 (nth index xs)) xs))").to_string(),
        "((4) (3 4) ((2) (3 4)))"
    );
}
...
#[test]
fn compiled_evaluates_dynamic_nth_pushnew_key_and_test_not() {
    assert_eq!(
        evaluate("(let ((xs (list (list (list 2 :a) (list 3 :b)))) (index 0)) (list (pushnew (list 2 :c) (nth index xs) :key #'car :test #'eql) (pushnew 4 (nth index xs) :test-not #'eql) xs))").to_string(),
...
#[test]
fn compiled_evaluates_nested_array_pushnew_options() {
    assert_eq!(
        evaluate(
            "(let ((xs (list (vector (list (list 1 :a))))))
               (list (pushnew (list 2 :b) (aref (car xs) 0) :key #'car :test #'eql)
                     (pushnew (list 2 :c) (aref (car xs) 0) :key #'car :test #'eql)
                     xs))",
        )
        .to_string(),
        "(((2 :B) (1 :A)) ((2 :B) (1 :A)) (#(((2 :B) (1 :A)))))"
