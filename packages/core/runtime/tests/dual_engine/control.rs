...
fn expands_loop_condition_clauses(#[case] eval_fn: EvalFn) {
    ...
    assert_eq!(
        evaluate("(loop for value in (list 1 2 3) when (evenp value) collect value)")
            .to_string(),
        "(2)"
    );
    assert_eq!(
        evaluate("(loop for value in (list 1 2 3) unless (evenp value) collect value)")
            .to_string(),
        "(1 3)"
    );
    assert_eq!(
...
fn expands_loop_for_then_clause(#[case] eval_fn: EvalFn) {
    ...
    assert_eq!(
        evaluate(r"(loop for value = 1 then (+ value 1) repeat 3 thereis (= value 2))")
            .to_string(),
        "T"
    );
    assert_eq!(
