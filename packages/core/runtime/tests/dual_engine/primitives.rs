...
#[case::evaluator(Runtime::eval_source as EvalFn)]
#[case::compiled(Runtime::eval_compiled_source as EvalFn)]
fn evaluates_optional_deftype_alias_parameters(#[case] eval_fn: EvalFn) {
    let result = evaluate_with(
        eval_fn,
        "(progn (deftype bounded-integer (low &optional (high *)) (integer low high))\
                (list (typep 3 '(bounded-integer 0 5))\
                      (typep 3 '(bounded-integer 0))\
                      (typep 8 '(bounded-integer 0))))",
    );
    assert_eq!(result.to_string(), "(T T T)");
}
...
#[case::evaluator(Runtime::eval_source as EvalFn)]
#[case::compiled(Runtime::eval_compiled_source as EvalFn)]
fn evaluates_numeric_folds(#[case] eval_fn: EvalFn) {
    assert_eq!(
        evaluate_with(eval_fn, "(list (min 12 8 4) (max 12 8 4) (gcd 84 30 18) (lcm 4 6 10))").to_string(),
        "(4 12 6 60)"
    );
}
...
#[case::evaluator(Runtime::eval_source as EvalFn)]
#[case::compiled(Runtime::eval_compiled_source as EvalFn)]
fn evaluates_integer_bitwise_helpers(#[case] eval_fn: EvalFn) {
    assert_eq!(
        evaluate_with(eval_fn, "(list (ash 3 2) (logtest 6 2) (logcount 13) (integer-length 13))").to_string(),
        "(12 T 3 4)"
    );
}
...
#[case::evaluator(Runtime::eval_source as EvalFn)]
#[case::compiled(Runtime::eval_compiled_source as EvalFn)]
fn evaluates_common_lisp_constantly(#[case] eval_fn: EvalFn) {
    let evaluate = |source: &str| evaluate_with(eval_fn, source);
    assert_eq!(
        evaluate("(list (funcall (constantly :answer)) (funcall (constantly 7) 1 2 3))")
            .to_string(),
        "(:ANSWER 7)",
    );
}
...
fn evaluates_sequence_operations_and_type_predicates(#[case] eval_fn: EvalFn) {
    ...
    assert_eq!(
        evaluate("(list (typep 1 'integer) (typep \"abc\" 'sequence) (typep \"abc\" 'vector) (typep \"abc\" 'array) (arrayp \"abc\") (array-rank \"abc\") (array-total-size \"abc\") (array-element-type \"abc\") (characterp #\\a) (keywordp :x) (vectorp #(1 2)) (vectorp \"abc\") (simple-vector-p \"abc\") (bit-vector-p #(0 1 1)) (simple-bit-vector-p #(0 1 1)) (bit-vector-p #(0 2)) (endp nil) (endp '(1)))").to_string(),
            "(T T T T T 1 3 CHARACTER T T T T NIL T T NIL T NIL)"
        );
}
