...
#[case::evaluator(Runtime::eval_source as EvalFn)]
#[case::compiled(Runtime::eval_compiled_source as EvalFn)]
fn evaluates_setf_mask_field_place(#[case] eval_fn: EvalFn) {
    let evaluate = |source: &str| evaluate_with(eval_fn, source);
    assert_eq!(
        evaluate(
            "(let ((x #xabc)) (list (setf (mask-field (byte 4 4) x) #x50) x (mask-field (byte 4 4) x)))"
        )
        .to_string(),
        "(80 2652 80)"
    );
...
fn evaluates_push_pop_and_psetf(#[case] eval_fn: EvalFn) {
    ...
    assert_eq!(
        evaluate(
            "(let ((first 0) (xs (list 1 2)))
                   (list (psetf first 7 (car xs) 8) first xs))",
        )
        .to_string(),
        "(8 7 (8 2))"
    );
