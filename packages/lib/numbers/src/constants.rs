...
const FLOAT_LEAST_POSITIVE: f64 = f64::MIN_POSITIVE;
const FLOAT_MOST_POSITIVE: f64 = f64::MAX;
...
/// Register the implementation-independent numeric constants supported by NCL.
pub fn register(ctx: &mut ThreadContext, runtime: &Runtime) -> Result<(), ObjectError> {
    ...
    for name in [
        ...
        set_float_constant(ctx, runtime, package, name, FLOAT_EPSILON)?;
    }
    for name in [
        ...
        set_float_constant(ctx, runtime, package, name, FLOAT_EPSILON / 2.0)?;
    }
    for name in [
        ...
        set_float_constant(ctx, runtime, package, name, FLOAT_LEAST_POSITIVE)?;
    }
    for name in [
    ...
    for name in [
        ...
        set_float_constant(ctx, runtime, package, name, FLOAT_MOST_POSITIVE)?;
    }
    for name in [
