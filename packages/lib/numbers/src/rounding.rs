...
fn number(ctx: &ThreadContext, word: Word) -> Result<Number, ObjectError> {
    match classify_object(ctx, word) {
        ...
        ObjectRef::Ratio(value) => {
            ...
            let numerator = integer(ctx, ratio_numerator(ctx, object)?)?;
            let denominator = integer(ctx, ratio_denominator(ctx, object)?)?;
...
fn as_float(value: Number) -> f64 {
    match value {
        ...
        Number::Ratio(numerator, denominator) => numerator as f64 / denominator as f64,
...
fn exact_round(value: Number, divisor: Number, mode: u8) -> Option<(Number, Number)> {
    let (value_n, value_d) = match value {
        ...
        Number::Ratio(numerator, denominator) => (numerator, denominator),
