...
use super::{Component, Device, Directory, DirectoryElement, DirectoryKind, Host, Pathname, PathnameError, Version};

pub fn parse_namestring(input: &str) -> Result<Pathname, PathnameError> {
    if input.is_empty() { return Err(PathnameError::EmptyNamestring); }
    let (host, rest) = split_prefix(input, ':')
        .map_or((None, input), |(value, rest)| (Some(Host(value.to_string())), rest));
    let (device, rest) = split_prefix(rest, ':')
        .map_or((None, rest), |(value, rest)| (Some(Device(value.to_string())), rest));
    let absolute = rest.starts_with('/');
    let trimmed = rest.trim_start_matches('/');
    let mut parts = trimmed.split('/').collect::<Vec<_>>();
    let file = if rest.ends_with('/') { None } else { parts.pop().filter(|part| !part.is_empty()) };
    let elements = parts.into_iter().filter(|part| !part.is_empty()).map(parse_directory_element).collect::<Result<Vec<_>, _>>()?;
    let (name, type_, version) = file.map(parse_file).transpose()?.unwrap_or((None, None, None));
    Ok(Pathname { host, device, directory: Some(Directory { kind: if absolute { DirectoryKind::Absolute } else { DirectoryKind::Relative }, elements }), name, type_, version })
}

fn split_prefix(input: &str, separator: char) -> Option<(&str, &str)> {
    let (prefix, rest) = match input.split_once(separator) {
        Some(parts) => parts,
        None => return None,
    };
...
fn parse_file(value: &str) -> Result<(Option<Component>, Option<Component>, Option<Version>), PathnameError> {
...
pub fn make_pathname(defaults: &Pathname, supplied: &Pathname) -> Pathname {
    Pathname {
        ...
        directory: supplied.directory.clone().or_else(|| defaults.directory.clone()),
        name: supplied.name.clone().or_else(|| defaults.name.clone()),
        type_: supplied.type_.clone().or_else(|| defaults.type_.clone()),
        version: supplied.version.clone().or_else(|| defaults.version.clone()),
    }
}
