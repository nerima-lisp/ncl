...
fn destructive_combine(
    ...
    while left_cursor != Word::NIL {
        ...
        let keep = keep_by_mode
            && !set_member(ctx, runtime, value, &result_values, options)?;
        if keep {
...
pub(crate) fn nunion(ctx: &mut ThreadContext, runtime: &Runtime, left: List, right: List, options: SetOptions) -> Result<Word, LispError> {
    destructive_combine(ctx, runtime, left, right, options, SetMode::Union)
}
pub(crate) fn nintersection(ctx: &mut ThreadContext, runtime: &Runtime, left: List, right: List, options: SetOptions) -> Result<Word, LispError> {
    destructive_combine(ctx, runtime, left, right, options, SetMode::Intersection)
}
pub(crate) fn ndifference(ctx: &mut ThreadContext, runtime: &Runtime, left: List, right: List, options: SetOptions) -> Result<Word, LispError> {
    destructive_combine(ctx, runtime, left, right, options, SetMode::Difference)
}
pub(crate) fn nexclusive_or(ctx: &mut ThreadContext, runtime: &Runtime, left: List, right: List, options: SetOptions) -> Result<Word, LispError> {
...
const SET_LISTS: &[Parameter] = &[set_parameter("first", ParameterType::List), set_parameter("second", ParameterType::List)];
const ADJOIN_ARGS: &[Parameter] = &[set_parameter("item", ParameterType::Any), set_parameter("list", ParameterType::List)];
...
fn set_descriptor(required: &'static [Parameter]) -> Builtin {
    Builtin { lambda_list: LambdaList::with_keys(required, SET_KEYS, false), convention: BuiltinConvention::Adapted }
}
...
fn set_binary(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, operation: SetOperation) -> Result<Word, ObjectError> {
    let values = set_args(args)?;
    let options = set_options(ctx, &values, 2).map_err(|error| { ctx.set_pending_lisp_error(error); ObjectError::TypeError })?;
    let left = set_list(ctx, values[0])?;
    let right = set_list(ctx, values[1])?;
    operation(ctx, runtime, left, right, options).map_err(|error| { ctx.set_pending_lisp_error(error); ObjectError::TypeError })
}
fn adjoin_builtin(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, _: &mut MultipleValues) -> Result<Word, ObjectError> {
    ...
    adjoin(ctx, runtime, values[0], list, options).map_err(|error| { ctx.set_pending_lisp_error(error); ObjectError::TypeError })
}
fn union_builtin(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, _: &mut MultipleValues) -> Result<Word, ObjectError> { set_binary(ctx, runtime, args, union) }
fn intersection_builtin(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, _: &mut MultipleValues) -> Result<Word, ObjectError> { set_binary(ctx, runtime, args, intersection) }
fn difference_builtin(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, _: &mut MultipleValues) -> Result<Word, ObjectError> { set_binary(ctx, runtime, args, difference) }
fn exclusive_or_builtin(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, _: &mut MultipleValues) -> Result<Word, ObjectError> { set_binary(ctx, runtime, args, exclusive_or) }
fn nunion_builtin(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, _: &mut MultipleValues) -> Result<Word, ObjectError> { set_binary(ctx, runtime, args, nunion) }
fn nintersection_builtin(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, _: &mut MultipleValues) -> Result<Word, ObjectError> { set_binary(ctx, runtime, args, nintersection) }
fn ndifference_builtin(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, _: &mut MultipleValues) -> Result<Word, ObjectError> { set_binary(ctx, runtime, args, ndifference) }
fn nexclusive_or_builtin(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, _: &mut MultipleValues) -> Result<Word, ObjectError> { set_binary(ctx, runtime, args, nexclusive_or) }
fn subsetp_builtin(ctx: &mut ThreadContext, runtime: &Runtime, args: &BuiltinArgs<'_>, _: &mut MultipleValues) -> Result<Word, ObjectError> { set_binary(ctx, runtime, args, subsetp) }
...
pub(crate) fn set_entry(name: &str) -> Option<BuiltinImplementation> {
    ...
    Some(BuiltinImplementation::adapted(set_descriptor(required), implementation, set_args))
}
use ncl_object::{
