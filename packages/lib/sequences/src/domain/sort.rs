...
fn call_designator(
    ...
    let function = match ncl_object::classify_object(ctx, designator) {
        ncl_object::ObjectRef::Function(function) => ncl_object::FunctionObject::try_from(function)
...
const SEQUENCE_PARAMETER: Parameter = Parameter {
    name: BuiltinName::new("sequence"),
    ty: ParameterType::Sequence,
};
const PREDICATE_PARAMETER: Parameter = Parameter {
    name: BuiltinName::new("predicate"),
    ty: ParameterType::FunctionDesignator,
};
const KEY_PARAMETER: Parameter = Parameter {
...
) -> Result<ncl_object::Word, ObjectError> {
    let values = pass(args)?;
    let sequence = super::sequence_value(ctx, values[0])?;
    let key = values
        .get(2)
        .copied()
        .filter(|word| *word != ncl_object::Word::NIL);
    ...
    Some(BuiltinImplementation::adapted(
        Builtin {
            lambda_list: LambdaList::with_rest(
                &[SEQUENCE_PARAMETER, PREDICATE_PARAMETER],
                KEY_PARAMETER,
            ),
