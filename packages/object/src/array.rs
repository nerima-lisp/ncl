...
mod tests {
    ...
    fn element_type_decoding_and_low_level_array_access_are_explicit() {
        for (value, expected) in [
            ...
            assert_eq!(ArrayElementType::from_word(Word::fixnum(value)), Ok(expected));
        }
        assert_eq!(
        ...
        let vector = make_simple_vector(&mut context, &runtime, &[Word::fixnum(3)])
            .unwrap_or(Word::NIL);
        assert_eq!(length(&context, vector, crate::widetag::SIMPLE_VECTOR, 0), Ok(1));
        assert_eq!(read(&context, vector, 1, crate::widetag::SIMPLE_VECTOR), Ok(Word::fixnum(3)));
        assert!(write(
            &mut context,
            vector,
            1,
        ...
        .is_ok());
        assert_eq!(read(&context, vector, 1, crate::widetag::STRING), Err(crate::ObjectError::TypeError));
        assert_eq!(length(&context, Word::NIL, crate::widetag::SIMPLE_VECTOR, 0), Err(crate::ObjectError::TypeError));
        let string = make_string(&mut context, &runtime, &['x']).unwrap_or(Word::NIL);
        assert_eq!(read(&context, string, 1, crate::widetag::STRING), Ok(Word::character('x' as u32)));
        assert_eq!(write(&mut context, string, 2, Word::NIL, crate::widetag::STRING), Err(crate::ObjectError::TypeError));
    }
}
