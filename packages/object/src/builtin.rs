...
/// Safe callback used by ABI wrappers generated by [`builtin!`].
pub type AbiBuiltin = fn(
    &mut ThreadContext,
    &BuiltinArgs<'_>,
    &mut MultipleValues,
) -> Result<Word, ObjectError>;
...
fn abi_pending_status(ctx: &mut ThreadContext) -> Option<NclStatus> {
    if let Some(error) = ctx.take_pending() {
        ctx.set_pending(error);
        return Some(NclStatus::Error);
    }
    if let Some(error) = ctx.take_pending_lisp_error() {
        ctx.set_pending_lisp_error(error);
        return Some(NclStatus::Error);
    }
    if let Some(condition) = ctx.take_pending_condition() {
        ctx.set_pending_condition(condition);
        return Some(NclStatus::Error);
    }
    if ctx.take_non_local_exit() {
        ctx.set_non_local_exit(true);
        return Some(NclStatus::NonLocalExit);
    }
    None
}
...
) -> Result<Word, NclStatus> {
    match callback(ctx, args, values) {
        Ok(result) => abi_pending_status(ctx).map_or(Ok(result), Err),
        Err(error) => {
            ctx.set_pending(error);
            Err(NclStatus::Error)
        }
...
macro_rules! builtin {
    ...
    ($name:ident, 0, $lambda_list:expr, $direct:ident, $variadic:ident, $callback:path) => { $crate::builtin!(@descriptor $name, 0, $lambda_list); $crate::builtin!(@direct_callback $direct, 0, $callback); $crate::builtin!(@variadic_callback $variadic, $callback); };
    ($name:ident, 1, $lambda_list:expr, $direct:ident, $variadic:ident, $callback:path) => { $crate::builtin!(@descriptor $name, 1, $lambda_list); $crate::builtin!(@direct_callback $direct, 1, $callback); $crate::builtin!(@variadic_callback $variadic, $callback); };
    ($name:ident, 2, $lambda_list:expr, $direct:ident, $variadic:ident, $callback:path) => { $crate::builtin!(@descriptor $name, 2, $lambda_list); $crate::builtin!(@direct_callback $direct, 2, $callback); $crate::builtin!(@variadic_callback $variadic, $callback); };
    ($name:ident, 3, $lambda_list:expr, $direct:ident, $variadic:ident, $callback:path) => { $crate::builtin!(@descriptor $name, 3, $lambda_list); $crate::builtin!(@direct_callback $direct, 3, $callback); $crate::builtin!(@variadic_callback $variadic, $callback); };
    ($name:ident, 4, $lambda_list:expr, $direct:ident, $variadic:ident, $callback:path) => { $crate::builtin!(@descriptor $name, 4, $lambda_list); $crate::builtin!(@direct_callback $direct, 4, $callback); $crate::builtin!(@variadic_callback $variadic, $callback); };
    ...
    (@direct_callback $name:ident, 0, $callback:path) => { pub extern "C" fn $name(ctx: *mut $crate::ThreadContext) -> $crate::Word { if ctx.is_null() { return $crate::Word::UNBOUND; } let mut values = $crate::MultipleValues::new(); let args = $crate::BuiltinArgs::new(&[]); match $crate::invoke_abi(unsafe { &mut *ctx }, $callback, &args, &mut values) { Ok(result) => result, Err(_) => $crate::Word::UNBOUND } } };
    (@direct_callback $name:ident, 1, $callback:path) => { pub extern "C" fn $name(ctx: *mut $crate::ThreadContext, a0: $crate::Word) -> $crate::Word { if ctx.is_null() { return $crate::Word::UNBOUND; } let mut values = $crate::MultipleValues::new(); let words = [a0]; let args = $crate::BuiltinArgs::new(&words); match $crate::invoke_abi(unsafe { &mut *ctx }, $callback, &args, &mut values) { Ok(result) => result, Err(_) => $crate::Word::UNBOUND } } };
    (@direct_callback $name:ident, 2, $callback:path) => { pub extern "C" fn $name(ctx: *mut $crate::ThreadContext, a0: $crate::Word, a1: $crate::Word) -> $crate::Word { if ctx.is_null() { return $crate::Word::UNBOUND; } let mut values = $crate::MultipleValues::new(); let words = [a0, a1]; let args = $crate::BuiltinArgs::new(&words); match $crate::invoke_abi(unsafe { &mut *ctx }, $callback, &args, &mut values) { Ok(result) => result, Err(_) => $crate::Word::UNBOUND } } };
    (@direct_callback $name:ident, 3, $callback:path) => { pub extern "C" fn $name(ctx: *mut $crate::ThreadContext, a0: $crate::Word, a1: $crate::Word, a2: $crate::Word) -> $crate::Word { if ctx.is_null() { return $crate::Word::UNBOUND; } let mut values = $crate::MultipleValues::new(); let words = [a0, a1, a2]; let args = $crate::BuiltinArgs::new(&words); match $crate::invoke_abi(unsafe { &mut *ctx }, $callback, &args, &mut values) { Ok(result) => result, Err(_) => $crate::Word::UNBOUND } } };
    (@direct_callback $name:ident, 4, $callback:path) => { pub extern "C" fn $name(ctx: *mut $crate::ThreadContext, a0: $crate::Word, a1: $crate::Word, a2: $crate::Word, a3: $crate::Word) -> $crate::Word { if ctx.is_null() { return $crate::Word::UNBOUND; } let mut values = $crate::MultipleValues::new(); let words = [a0, a1, a2, a3]; let args = $crate::BuiltinArgs::new(&words); match $crate::invoke_abi(unsafe { &mut *ctx }, $callback, &args, &mut values) { Ok(result) => result, Err(_) => $crate::Word::UNBOUND } } };
    (@variadic_callback $name:ident, $callback:path) => { pub extern "C" fn $name(ctx: *mut $crate::ThreadContext, argc: usize, args: *const $crate::Word, values: *mut $crate::MultipleValues) -> $crate::NclStatus { if ctx.is_null() || (argc != 0 && args.is_null()) || values.is_null() { return $crate::NclStatus::Error; } let words = if argc == 0 { &[] } else { unsafe { std::slice::from_raw_parts(args, argc) } }; let args = $crate::BuiltinArgs::new(words); match $crate::invoke_abi(unsafe { &mut *ctx }, $callback, &args, unsafe { &mut *values }) { Ok(result) => { unsafe { (*values).set(&[result]); } $crate::NclStatus::Ok }, Err(status) => status } } };
}
