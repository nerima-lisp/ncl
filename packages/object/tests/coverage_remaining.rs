...
fn nested_displacement_and_structure_metadata_have_observable_results() {
    ...
    assert_eq!(
        array_row_major_ref(&context, nested, 0),
        Ok(Word::fixnum(3))
    );
    ...
    assert_eq!(
        structure_ref(&context, structure, 2),
    ...
    assert_eq!(
        structure_set(&mut context, Word::NIL, 0, Word::NIL),
...
fn package_visibility_and_list_removal_cover_internal_external_and_inherited() {
    ...
    let consumer =
        Package::new(&mut context, &runtime, "CONSUMER").unwrap_or_else(|_| panic!("consumer"));
    ...
    assert_eq!(
        owner
            .name(&context)
            .and_then(|word| ncl_object::string_ref(&context, word, 0)),
        Ok('O')
    );
    let (symbol_a, status) = owner
        .intern(&mut context, &runtime, "A")
        .unwrap_or_else(|_| panic!("intern"));
...
fn runtime_context_roots_and_classification_report_values() {
    ...
    assert_eq!(
        classify_object(&context, Word::NIL),
        ObjectRef::Symbol(Word::NIL)
    );
