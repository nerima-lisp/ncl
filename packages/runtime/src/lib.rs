...
impl Runtime {
    ...
    pub fn eval(&mut self, source: &str) -> Result<Word, RuntimeError> {
        let options = ReadOptions::standard(&mut self.context, &self.object)?;
        let mut input = StringSource::new(source);
        let mut forms = Vec::new();
        while let Some(form) = read(&mut self.context, &self.object, &mut input, &options)? {
            forms.push(form);
        }
    ...
    fn compile_form(&mut self, form: Word) -> Result<Word, RuntimeError> {
        ...
        let compiled = if cfg!(target_arch = "aarch64") {
            ncl_codegen::compile_function_aarch64(&lowered.entry, &abi)
        } else {
            ncl_codegen::compile_function_x86_64(&lowered.entry, &abi)
        }
