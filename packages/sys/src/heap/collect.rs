...
struct WeakMarkContext<'a> {
    state: &'a super::State,
    full: bool,
    live: &'a mut HashSet<usize>,
    stack: &'a mut Vec<usize>,
    weak_kv: &'a mut HashMap<usize, Weakness>,
...
impl WeakMarkContext<'_> {
    fn drain(&mut self) {
        loop {
            while let Some(index) = self.stack.pop() {
                ...
                if super::Heap::is_hash_table(self.state, index)
                    && let Some(weakness) = super::Heap::hash_table_weakness(self.state, index)
                {
                ...
                for slot in scan::layout(self.state, index) {
                    ...
                    if let Some(value) = self.state.objects[index]
                        ...
                        .copied()
                        .map(Word::from_bits)
                        && let Some(next) = super::Heap::find(self.state, value)
                    {
        ...
        for slot in scan::layout(self.state, table) {
            if slot != HASH_TABLE_KV
                && let Some(next) = words
                    ...
                    .copied()
                    .map(Word::from_bits)
                    .and_then(|value| super::Heap::find(self.state, value))
            {
                self.stack.push(next);
            }
        }
    ...
    fn mark_value(&mut self, value: Word) {
        if let Some(index) = super::Heap::find(self.state, value) {
            self.stack.push(index);
        }
    }

    fn referent_is_live(&self, value: Word) -> bool {
